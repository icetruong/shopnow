...
Đồng bộ convention với order-service:

```properties
spring.application.name=shipping-service
server.port=8087

# JWT (chung public key với các service khác)
...
# Kafka — GIỐNG order-service: producer JSON, consumer nhận String rồi tự parse bằng ObjectMapper
...
# Carrier — GHN
carrier.ghn.base-url=https://dev-online-gateway.ghn.vn/shiip/public-api
carrier.ghn.token=${GHN_TOKEN}
carrier.ghn.shop-id=${GHN_SHOP_ID}
carrier.ghn.webhook-secret=${GHN_WEBHOOK_SECRET}
carrier.ghn.default-service-id=53320

# Carrier — GHTK
carrier.ghtk.base-url=https://services.giaohangtietkiem.vn
carrier.ghtk.token=${GHTK_TOKEN}
carrier.ghtk.webhook-secret=${GHTK_WEBHOOK_SECRET}

# Mặc định khi tạo vận đơn
shipping.default-carrier=GHN

# Kho gửi hàng (điểm "from" khi tính phí / tạo đơn)
shipping.from.district-id=1442
shipping.from.ward-code=21012
shipping.from.address=Kho ShopNow Quận 1, TP.HCM

# Ước lượng khối lượng / kích thước (TẠM THỜI — tới khi Product Service expose weight thật)
shipping.default-item-weight-grams=500
shipping.default-package.length-cm=20
shipping.default-package.width-cm=15
shipping.default-package.height-cm=10
```
